   ...
   if(!isset($_COOKIE["token"])){
      ...
      if (verificationToken($decoded_array)){
        if($decoded_array['role']==="joueur"){
          ...
          if(isset($_POST['typeCompet']) && !empty($_POST['typeCompet'])){
            $type=htmlspecialchars($_POST['typeCompet']);
            $idType=getIdType($type);
            $competitions=getCompetitionsType($idType);
            require_once('../view/participerCompetitionType.php');
          }
          else{
